...
import { CheckCircle2, Church, Download, Info, Loader2, Lock, User, Wallet } from 'lucide-react'
...
import { Step1License } from './wizard/step-1-license'
...
import { Step5FinishWeb } from './wizard/step-5-finish-web'

// A wizard teljes kliens-oldali állapota.
// Standalone-ban a Step 1 tölti fel az auth / license mezőket.
// Web módban ezek üresek maradnak (ott a session-ből jön az auth).
export interface WizardData {
  // Step 1 — Licensz (csak standalone)
  email: string
  password: string
  userId: string | null
...
const STEPS = [
  { id: 1, title: 'Licensz', icon: Lock, description: 'Bejelentkezés és aktiválás' },
  ...
  { id: 5, title: 'Kész', icon: Download, description: 'Adatok letöltése' },
]

interface WelcomeWizardClientProps {
  /** 'standalone' = Step 1 (licensz) aktiválva, 'web' = Step 1 kimarad */
  mode?: 'standalone' | 'web'
}

export function WelcomeWizardClient({ mode = 'standalone' }: WelcomeWizardClientProps = {}) {
  const router = useRouter()
  const [currentStep, setCurrentStep] = useState(mode === 'web' ? 2 : 1)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if ('error' in result) {
        // Web módban a server action hívás hibázhat (nincs user pl. standalone-ban),
        // csendben fallbackolunk a kezdő állapotra — a Step 1 vagy az auth oldja meg.
        setLoading(false)
        return
      }
      ...
      // Web módban soha ne Step 1-en legyünk (ott a licensz aktiválás,
      // ami csak standalone módban releváns). Ha valamiért ott van a mentés,
      // lépjünk Step 2-re.
      const savedStep = wp.current_step || 1
      const effectiveStep = mode === 'web' && savedStep < 2 ? 2 : savedStep
  ...
  // ─── Mentés + léptetés — az A fázis lényege ───
  // A Step 1 (licensz) továbbra is a standalone /api/standalone/activate-en megy,
  // NEM ezen a server action-ön. A Step 2-4 adatai viszont itt perzisztálódnak.
      ...
      <div className="card-raised overflow-hidden">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentStep === 5 &&
              (mode === 'web' ? <Step5FinishWeb /> : <Step5Finish data={data} />)}
          </motion.div>
        </AnimatePresence>
      </div>
