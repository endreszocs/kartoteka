      ...
      <div className="border rounded-lg overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b">
            <tr>
              {activeTab === 'keresztseg' && <><th className="p-2 text-left cursor-pointer" onClick={() => handleSort('okirat')}>Okirat</th><th className="p-2 text-left">Név</th><th className="p-2 text-left cursor-pointer" onClick={() => handleSort('datum')}>Dátum</th><th className="p-2 text-left hidden md:table-cell">Lelkész</th></>}
              {activeTab === 'konfirmalas' && <><th className="p-2 text-left">Név</th><th className="p-2 text-left cursor-pointer" onClick={() => handleSort('datum')}>Dátum</th><th className="p-2 text-left hidden md:table-cell">Lelkész</th></>}
              {activeTab === 'hazassag' && <><th className="p-2 text-left">Vőlegény</th><th className="p-2 text-left">Menyasszony</th><th className="p-2 text-left cursor-pointer" onClick={() => handleSort('datum')}>Dátum</th></>}
          ...
          <tbody>
            {filtered.map(d => (
              <tr key={d.id} className="border-b hover:bg-slate-50">
                {activeTab === 'keresztseg' && <><td className="p-2 text-xs text-muted-foreground">{d.okirat || '—'}</td><td className="p-2 font-medium">{getName(d)}</td><td className="p-2 text-muted-foreground">{getDate(d)}</td><td className="p-2 hidden md:table-cell text-muted-foreground">{d.lelkeszneve || '—'}</td></>}
                {activeTab === 'konfirmalas' && <><td className="p-2 font-medium">{getName(d)} {d.szemely?.ferfi ? '♂' : '♀'}</td><td className="p-2 text-muted-foreground">{getDate(d)}</td><td className="p-2 hidden md:table-cell text-muted-foreground">{d.lelkeszneve || '—'}</td></>}
                {activeTab === 'hazassag' && <><td className="p-2 font-medium">{d.ferfi ? `${d.ferfi.csaladnev} ${d.ferfi.k_nev}` : '—'}</td><td className="p-2 font-medium">{d.no ? `${d.no.csaladnev} ${d.no.k_nev}` : '—'}</td><td className="p-2 text-muted-foreground">{getDate(d)}</td></>}
