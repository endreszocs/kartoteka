...
/**
 * Oblio ellenőrzés fül.
 *
 * Helyileg parse-olja az Oblio Wallet-ből letöltött befogadott e-Factura
 * UBL XML-eket, és összeveti a `kiadas` rekordokkal.
 *
 * Adatfolyam:
 *   1. Helyi mappa scan (jszip ZIP kibontás → XML extraction)
 *   2. UBL XML parse (browser DOMParser)
 *   3. IndexedDB cache (Dexie)
 *   4. Server-oldali párosítás (oblio_kiadas_match) lekérés
 *   5. Pure matcher algoritmus → eredmény-lista
 *   6. UI render
 */
...
import {
  getOblioBefogadottDir,
  getOblioFeldolgozvaDir,
  getOblioFolderStatus,
  listOblioFolderFiles,
  ...
  type OblioFolderStatus,
  type OblioLocalFile,
} from '@/lib/finance/oblio/oblio-folder'
import {
  batchMatchPdfsToXmls,
  type ContentMatchResult,
} from '@/lib/finance/oblio/pdf-xml-content-matcher'
import {
  batchMatchPdfsToXmlsByName,
  type NameMatchResult,
} from '@/lib/finance/oblio/pdf-xml-name-matcher'
import {
  parseUblXml,
  extractAnafUuidFromFilename,
  normalizeFileBaseName,
  type UblInvoiceMeta,
} from '@/lib/finance/oblio/ubl-parser'
import {
  getCachedXml,
  putCachedXml,
  pruneCacheToActualFiles,
} from '@/lib/finance/oblio/oblio-cache'
import {
  matchXmlsToKiadas,
  type ExistingMatch,
  type MinimalKiadas,
...
} from '@/lib/finance/oblio/oblio-matcher'
import {
...
import { OblioEllenorzesFolderCard } from './oblio-ellenorzes-folder-card'
import {
  OblioEllenorzesWarnings,
  OblioFormatGuideCard,
} from './oblio-ellenorzes-warnings'
...
import { OblioKiadasWizardDialog, type WizardXmlItem } from '@/components/modals/oblio-kiadas-wizard-dialog'
...
export function OblioEllenorzesTab({ congregationSlug, congregationName, currentYear }: Props) {
  ...
  const loadFolderStatus = useCallback(async () => {
    const st = await getOblioFolderStatus(congregationSlug)
    setFolderStatus(st)
    return st
  }, [congregationSlug])

  const loadDbData = useCallback(async () => {
    const res = await listOblioMatchesAndKiadasok(currentYear)
    if (res.error) {
      toast.error(`DB lekérés: ${res.error}`)
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (!st.supported) {
        toast.error('A böngésző nem támogatja a File System Access API-t.')
        return
      }
      if (!st.hasRoot) {
        toast.error('Először állítsd be a KARTOTEKA helyi mappát.')
        return
      }

      // 3.2 Mappa-handle-ek
      const befogadottDir = await getOblioBefogadottDir(congregationSlug)
      const feldolgozvaDir = await getOblioFeldolgozvaDir(congregationSlug)
      if (!befogadottDir || !feldolgozvaDir) {
        toast.error('A mappák nem érhetők el. Ellenőrizd a permission-t.')
        return
      }

      // 3.3 ZIP-ek kibontása
      const zipResults = await processAllZipsInFolder(befogadottDir, feldolgozvaDir)
      ...
      if (zipResults.length > 0) {
        ...
        if (errs.length > 0) {
          toast.warning(`${zipResults.length} ZIP feldolgozva, ${errs.length} hiba.`)
        } else if (totalExtracted > 0) {
          toast.success(`${zipResults.length} ZIP kibontva (${totalExtracted} fájl).`)
        }
      }
      ...
      for (const f of xmlsRaw) {
        ...
        // Parse
        try {
          const text = await readFileAsText(f.handle)
      ...
      if (dupFilesToDelete.length > 0) {
        try {
          const res = await removeFilesFromFolder(befogadottDir, dupFilesToDelete)
          if (res.deleted > 0) {
            toast.success(
      ...
      if (toPersist.length > 0) {
        const persistRes = await bulkSaveOblioMatches(toPersist)
        if (persistRes.errors.length > 0) {
          toast.warning(`${persistRes.saved} auto-match mentve, ${persistRes.errors.length} hiba.`)
        }
      }
      ...
      if (pdfFiles.length > 0) {
        ...
        if (orphanPdfs > 0) {
          ...
          toast.success(`${pdfFiles.length} PDF mind biztosan párosítva.`)
        }
      ...
      if (hadNewFiles) {
        ...
        const dlRes = await recordOblioDownloadNow(latestIso)
        if (!dlRes.error) {
          setUtolsoLetoltes(latestIso)
        }
      }

      toast.success(`Frissítve: ${built.length} XML, ${built.filter((b) => b.match.kiadasId !== null).length} párosítva.`)
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e)
      toast.error(`Frissítés hiba: ${msg}`)
    } finally {
      setRefreshing(false)
    }
  ...
  async function handleOpenXml(e: EnrichedXml) {
    try {
      await openLocalFileInBrowser(e.xmlHandle)
    } catch (err) {
      toast.error(`Nem nyitható meg: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
  ...
  async function handleDeleteUnknownFiles() {
    ...
    setCleaningUp(true)
    try {
      const dir = await getOblioBefogadottDir(congregationSlug)
      if (!dir) {
        toast.error('A mappa nem érhető el.')
        return
      }
      const res = await removeFilesFromFolder(
        dir,
        unknownFiles.map((f) => f.name),
      )
      if (res.errors.length > 0) {
        toast.warning(`${res.deleted} fájl törölve, ${res.errors.length} hiba.`)
      } else {
        toast.success(`${res.deleted} ismeretlen fájl törölve.`)
      }
      setUnknownFiles([])
  ...
  async function handleContentMatchOrphanPdfs() {
    if (orphanPdfs.length === 0) {
      toast.info('Nincsenek árva PDF-ek a feldolgozáshoz.')
      return
    }
    ...
    try {
      ...
      toast.info(`Fájlnév-minta párosítás indul (${orphanPdfs.length} fájl)…`)
      ...
      for (let i = 0; i < orphanPdfs.length; i++) {
        ...
        if (r.xmlFileName && (r.confidence === 'high' || r.confidence === 'medium')) {
          ...
          const renRes = await renameFileInFolder(dir, pdf.name, newPdfName)
          if (renRes.success) {
            renamedByName++
          } else {
            failed++
      ...
      if (stillOrphan.length > 0) {
        ...
        for (const r of contentResults) {
          if (r.xmlFileName && r.confidence !== 'low') {
            const xmlBase = r.xmlFileName.replace(/\.xml$/i, '')
            const newPdfName = `${xmlBase}.pdf`
            const renRes = await renameFileInFolder(dir, r.pdfName, newPdfName)
            if (renRes.success) {
              renamedByContent++
            } else {
              failed++
  ...
  async function handleReprocessArchive() {
    ...
    try {
      ...
      const feldolgozvaDir = await getOblioFeldolgozvaDir(congregationSlug)
      if (!befogadottDir || !feldolgozvaDir) {
        toast.error('A mappák nem elérhetőek.')
        return
      }
      const res = await reprocessZipsFromArchive(befogadottDir, feldolgozvaDir)
      if (res.error) {
        toast.error(`Újra-feldolgozás hiba: ${res.error}`)
        return
      }
      toast.success(
  ...
  async function handleRemoveMatch(matchUuid: string) {
    ...
    if (!dbMatch) {
      toast.error('A párosítás nem található a DB-ben (csak helyileg auto-matched).')
      return
    }
    const ok = window.confirm('Biztosan törlöd a párosítást?')
    if (!ok) return
    const res = await removeOblioMatch(dbMatch.id)
    if (res.error) {
      toast.error(res.error)
      return
    }
    toast.success('Párosítás törölve.')
    await loadDbData()
    void handleRefresh()
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={_topGridClass}>
        ...
        {folderStatus ? (
          <OblioEllenorzesFolderCard
            ...
            onOpenSettings={() => {
              window.location.href = '/profile?tab=offline'
            }}
        ...
        {_showWizardTrigger && (
          <div className="card-raised border border-amber-200 bg-gradient-to-br from-amber-50/60 to-yellow-50/40 p-4 sm:p-5 flex flex-col gap-3">
            ...
            <Button
              ...
              className="rounded-xl bg-amber-600 text-white hover:bg-amber-700 shadow-sm w-full sm:w-auto sm:self-end"
            >
        ...
        <div className="card-raised border border-rose-200 bg-rose-50/40 p-4 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-3">
            ...
            <Button
              ...
              className="shrink-0 rounded-xl bg-rose-600 text-white hover:bg-rose-700 shadow-sm"
            >
              {contentMatching ? (
                <>
              ...
              {orphanPdfs.map((pdf) => (
                ...
                >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => openLocalFileInBrowser(pdf.handle)}
      ...
      {folderStatus?.ellenorzesDirReady && filteredDiag.length > 0 && (
        <div className="card-raised border border-orange-200 bg-orange-50/40 p-3 sm:p-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            ...
            variant="outline"
            className="rounded-xl border-orange-300 text-orange-800 hover:bg-orange-100 shrink-0"
          >
      ...
      {/* Nyomtatási központ dialog — a leltár mintára: iframe előnézet,
          közvetlen PDF nyomtatás, ha van; egyébként generált HTML */}
      ...
      {/* Párosítás-diagnosztika dialog — 3 fülre bontva (70-100%, 40-69%, <40%).
          Minden fülben lehet egyesével párosítani / mellőzni, vagy tömegesen. */}
      ...
      {/* Kiadás-bevezetés wizard — kronológiai végigvezetés a hiányzó kiadásokon */}
...
function MatchBadge({ e }: { e: EnrichedXml }) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <Badge variant="secondary" className={`${colors[e.match.confidence]} hover:opacity-90`}>
  ...
  if (!hasAnyXml) {
    return (
      <div className="card-raised p-8 text-center">
        ...
        <Button variant="outline" className="mt-4 rounded-xl" onClick={onRefresh}>
