...
// ⚠️ DEPRECATED — Ez a fájl a god mode legacy v2 verziója. Az aktív kód az
// `actions-v4.ts`. Ezt a fájlt csak akkor használja a rendszer, ha valamelyik
// régi UI komponens (admin-tabs-v2, security-settings-tab, god-mode-banner-v2,
// god-mode-dialog-v2, god-mode-dialog-v3) éles használatba kerül vissza.
...
async function readStoredPin(supabase: Awaited<ReturnType<typeof createClient>>) {
  ...
  if (result.error) {
    if (isMissingRelationError(result.error)) {
      return {
        ...
        schemaReady: false,
        warning: envPin
          ? 'A tartós God Mode PIN tároláshoz még futtatni kell a kiegészítő SQL-bővítést. A rendszer az env változóra esik vissza.'
    ...
    if (isPermissionError(result.error)) {
      return {
        ...
        schemaReady: true,
        warning: envPin
          ? 'A God Mode PIN beállítási tábla létezik, de az olvasás nem sikerült. Az env változóra esik vissza.'
          : 'A God Mode PIN beállítási tábla létezik, de az olvasás nem sikerült, és nincs env fallback.',
      }
    }
  ...
  return {
    ...
    source: (envPin ? 'env' : 'none') as PinSource,
    schemaReady: true,
    warning: envPin ? undefined : 'A God Mode PIN nincs beállítva sem az adatbázisban, sem a GOD_MODE_PIN env változóban.',
  }
}
...
export async function activateGodMode(pin: string) {
  ...
  if (!isValidPin(cleanedPin)) {
    return { error: 'A God Mode PIN pontosan 6 számjegyből kell álljon.' }
  }
  ...
  if (!storedPin.pin) {
    return {
      error:
        storedPin.warning ||
        'A God Mode PIN nincs konfigurálva. Állítsd be a GOD_MODE_PIN környezeti változót, vagy konfiguráld a system_settings táblát.',
    }
  }
...
export async function updateGodModePin(pin: string) {
  ...
  if (result.error) {
    ...
    if (isPermissionError(result.error)) {
      return { error: 'Az adatbázis-jogosultság jelenleg nem engedi a God Mode PIN mentését.' }
    }

    return { error: result.error.message }
  }

  return { success: 'A God Mode PIN sikeresen frissült.' }
}
