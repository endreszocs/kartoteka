...
/**
 * @deprecated A `previewFxAtYearStart` a HIBÁS kö́nyvelési logikán alapult
 * (januárba könyvelte volna az FX-et). Használd helyette a
 * `checkYearStartState`-t, ami a helyes év végi FX revaluation gyakorlatot
 * tükrözi. Ez a függvény csak kompatibilitás miatt marad — teljes eltávolítás
 * után törölhető.
 */
export async function previewFxAtYearStart(args: {
  bankszamlaId: number
  eve: number
  newYearValuta: number
  newYearArfolyam: number
}): Promise<{ data?: FxYearStartPreview; error?: string }> {
  return { error: 'A január 1-i FX könyvelés könyvelésileg hibás. Használd a checkYearStartState-et és az év végi FX revaluation dialogot.' }
}

/** @deprecated — backward compat typedef */
export interface FxYearStartPreview {
  valuta: string
  prevYearNyitoRon: number
  prevYearIncomeRon: number
  prevYearExpenseRon: number
  prevYearClosingRon: number
  newYearValuta: number
  newYearArfolyam: number
  newYearRon: number
  fxDiff: number
  fxIrany: 'nyereseg' | 'veszteseg' | 'nulla'
  hasPreviousYear: boolean
}

/**
 * @deprecated HIBÁS KÖNYVELÉSI LOGIKA — ne használd!
 *
 * A román OMFP 1802/2014 és a magyar egyházi számvitel szerint az FX
 * átértékelést DECEMBER 31-I dátummal kell könyvelni (év végi revaluation),
 * NEM január 1-i dátummal. Az új év január 1-i RON nyitó egyenleg =
 * a december 31. átértékelt záró RON — ugyanaz az érték, nincs különbség.
 *
 * A helyes út: használd a meglévő `FxRevaluationDialog`-ot az előző év
 * december 31-re, és utána add meg az új év nyitóját (akkor nincs FX diff).
 *
 * Ez a függvény kompatibilitás okán megmarad, de hibajelzést ad vissza.
 */
export async function applyFxAtYearStart(args: {
  bankszamlaId: number
  eve: number
  fxDiff: number
  arfolyam: number
  ...
  return {
    error:
      'Januári FX könyvelés könyvelésileg hibás. A helyes gyakorlat: december 31-re kell az FX revaluation (előző év Bank fül → „Évvégi átértékelés"). Utána az új év január 1. nyitó RON egyenleg = december 31. átértékelt záró RON (ugyanaz az érték).',
  }
}

// (belső helper — a kódban maradt, de a most deprecated applyFxAtYearStart nem
// hivatkozik rá; a meglévő fx-revaluation-dialog.tsx által használt legacy
// impl futott itt, törölve a hibás viselkedés miatt)
async function _legacyApplyFxAtYearStartRemoved(args: {
  bankszamlaId: number
  eve: number
  fxDiff: number
  arfolyam: number
  ...
  if (Math.abs(args.fxDiff) < 0.005) {
    return { error: 'Nincs érdemleges árfolyam-különbség.' }
  }

  const congregationId = access.effectiveCongregationId
  const userId = access.user.id
  const datum = `${args.eve}-01-01`

  if (args.fxDiff > 0) {
    // Árfolyam nyereség → befizetes 103.04
    const { data: cel } = await access.supabase
      .from('befizetescel')
      .select('id, szamadasicel!id_szamadasicel(kod)')
      .eq('congregation_id', congregationId)
      .eq('aktiv', true)

    const target = (cel || []).find((c) => {
      const s = c.szamadasicel as { kod?: string } | { kod?: string }[] | null
      const kod = Array.isArray(s) ? s[0]?.kod : s?.kod
      return kod === '103.04'
    }) || (cel || []).find((c) => {
      const s = c.szamadasicel as { kod?: string } | { kod?: string }[] | null
      const kod = Array.isArray(s) ? s[0]?.kod : s?.kod
      return kod?.startsWith('103')
    })

    if (!target) {
      return {
        error: 'Nem található "Árfolyam nyereség" kategória (103.04). Előbb hozd létre a Költségvetésben vagy kategóriakezelőben.',
      }
    }

    const { error } = await access.supabase.from('befizetes').insert({
      osszeg: args.fxDiff,
      osszeg_ron: args.fxDiff,
      arfolyam: args.arfolyam,
      datum,
      id_befizetescel: (target as { id: number }).id,
      id_szemely: null,
      id_csalad: null,
      forrasa: `FX átértékelés ${args.eve}. január 1.`,
      iratszam: `FX-${args.eve}-01-01`,
      irattipus: 'banki',
      bankszamla_id: args.bankszamlaId,
      megjegyzes: args.megjegyzes || `Év eleji FX átértékelés (árfolyam nyereség) — ${args.arfolyam} RON/egység`,
      deleted: false,
      congregation_id: congregationId,
      fizetettev: args.eve,
      is_potlas: false,
    } as Record<string, unknown>)
    if (error) return { error: `Könyvelés hiba: ${error.message}` }
  } else {
    // Árfolyam veszteség → kiadas 203.03
    const { data: cel } = await access.supabase
      .from('kiadascel')
      .select('id, szamadasicel!id_szamadasicel(kod)')
      .eq('congregation_id', congregationId)
    ...
    const target = (cel || []).find((c) => {
      ...
      const s = c.szamadasicel as { kod?: string } | { kod?: string }[] | null
      const kod = Array.isArray(s) ? s[0]?.kod : s?.kod
      return kod?.startsWith('203')
    })

    if (!target) {
      return {
        error: 'Nem található "Árfolyam veszteség" kategória (203.03). Előbb hozd létre a Költségvetésben vagy kategóriakezelőben.',
      }
    }

    const absAmount = Math.abs(args.fxDiff)
    const { error } = await access.supabase.from('kiadas').insert({
      osszeg: absAmount,
      osszeg_ron: absAmount,
      arfolyam: args.arfolyam,
      datum,
      id_kiadascel: (target as { id: number }).id,
      kedvezmenyzett: `FX átértékelés ${args.eve}. január 1.`,
      atvevo: `FX átértékelés ${args.eve}. január 1.`,
      iratszam: `FX-${args.eve}-01-01`,
      irattipus: 'banki',
      bankszamla_id: args.bankszamlaId,
      megjegyzes: args.megjegyzes || `Év eleji FX átértékelés (árfolyam veszteség) — ${args.arfolyam} RON/egység`,
      deleted: false,
      congregation_id: congregationId,
      userid: userId,
      xkey: globalThis.crypto?.randomUUID?.() ?? Math.random().toString(36).slice(2),
      nyugta: `FX-${args.eve}-01-01`,
    } as Record<string, unknown>)
    if (error) return { error: `Könyvelés hiba: ${error.message}` }
  }
