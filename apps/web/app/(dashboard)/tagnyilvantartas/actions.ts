...
export async function getMembers(): Promise<{
  ...
  const members: EnrichedMember[] = memberRowsForList.map(m => {
...
export async function getMemberDetails(id: number, familyId?: number | null) {
  const { supabase, congregationId } = await getProfileCongregation()
  const [memberRes, kereszt, konfirm, hazassagRes, temetesRes, bekolt, attert, payments, familyPayments, yearlySettingsRes, exemptionsRes, discountsRes] = await Promise.all([
    congregationId
