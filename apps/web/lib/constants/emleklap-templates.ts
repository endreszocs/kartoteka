...
// 2026-05-30 (Hagyományos kék — Újragondolt spec): arany dísz-szín a
// gyülekezet-névhez és „EMLÉKLAP” alcímhez, fő-szöveg sötét fekete.
const COLOR_GOLD_KEK = '#B48A3A'
const COLOR_BLACK_TITLE = '#222222'
const COLOR_BLACK_BODY = '#2F2F2F'

// 2026-05-30 v3: a „Hagyományos kék" sablon a friss „Újragondolt" spec szerinti
// pixel-pontos elrendezéssel. Vászon: 1448×2047 px (A4 portrait).
// Kódolás: minden koordináta % a vászon szélességéhez (x, width) ill. magasságához
// (y, fontSize) képest. A spec szerint mindkét középre rendezett mező BAL felső
// sarka adott, így a x értékek közvetlenül átvehetők (1448-as bázis → /14.48).
// Színek: arany dísz (#B48A3A) a gyülekezet-névhez + EMLÉKLAP alcímhez,
// fő-fekete (#222) a címhez/névhez/aláíráshoz, lágyabb fekete (#2F2F2F) a body-hoz.
const KERESZTELESI_KEK_FIELDS: EmleklapField[] = [
  // 6.1 Gyülekezet neve — arany, Cinzel, ls=5px ≈ 0.15em @ 33px
  {
    id: 'congregationName',
  ...
  // 6.2 Főcím 1. sor „KERESZTELŐI" — Cormorant Garamond SemiBold, font 92px → 4.49%
  {
    id: 'titleLine1',
    label: 'Főcím – 1. sor',
  ...
  // 6.3 Alcím „EMLÉKLAP" — Cinzel, arany, font 36px → 1.76%, ls=10px ≈ 0.28em
  {
    id: 'titleLine2',
  ...
  // 6.4 Köszöntő „BÜSZKÉN KÖSZÖNTJÜK" — Cormorant Garamond Medium, ls=4px ≈ 0.11em
  {
    id: 'greeting',
    label: 'Köszöntő sor',
    ...
    fontFamily: FONT_SERIF,
    fontWeight: 500,
    color: COLOR_BLACK_BODY,
    textAlign: 'center',
    lineHeight: 1.25,
    letterSpacing: 0.11,
    textTransform: 'uppercase',
  },
  // 6.5 Keresztelt neve — Great Vibes kalligráfia, font 72px → 3.52%
  {
    id: 'fullName',
    ...
    lineHeight: 1.08,
    letterSpacing: 0.014,
    multiline: true, // hosszú név auto-wrap
  },
  // 6.6 „testvérünket" — Cormorant Regular, ls=2px ≈ 0.056em @ 36px
  {
    id: 'relativeLabel',
  ...
  // 6.7–6.12 Törzsszöveg — 6 sor, manuális sortörés a placeholder-bekezdésben.
  // A spec szerint min. 5 sor (justify-center), wrap-pelve. x=171, w=1107 px →
  // 11.81% / 76.45%. font 32px → 1.56%. line-height 40px → lineHeight=1.25.
  {
    id: 'paragraph',
  ...
  // 6.13 Hely és dátum — font 36px, ls=2px → 0.056em
  {
    id: 'placeAndDate',
    label: 'Kelt (hely + dátum)',
    defaultValue: '{{issueLocation}}, {{issueDate}}',
  ...
  // 6.14 Bal név — Cormorant SemiBold, font 28px → 1.37%, ls=1px ≈ 0.036em
  {
    id: 'pastorName',
    label: 'Lelkipásztor neve',
    defaultValue: '{{pastorName}}',
  ...
  // 6.15 Bal tisztség — font 20px → 0.98%, ls=1px ≈ 0.05em
  {
    id: 'pastorRole',
    label: 'Lelkipásztor titulus',
    defaultValue: 'LELKIPÁSZTOR',
