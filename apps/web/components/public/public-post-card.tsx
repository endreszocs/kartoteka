    ...
    >
      ...
      {post.cover_image_url ? (
        ...
        >
          ...
          >
            <defs>
              <pattern id={`card-pattern-${post.id}`} x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
                <circle cx="20" cy="20" r="1" fill="white" />
                <circle cx="0" cy="0" r="0.5" fill="white" />
              </pattern>
            </defs>
            <rect width="100%" height="100%" fill={`url(#card-pattern-${post.id})`} />
          </svg>
          <div className="absolute inset-0 flex items-center justify-center">
            <div
              className="font-serif text-6xl sm:text-7xl font-bold opacity-30 text-white"
            >
              {post.title.charAt(0)}
            </div>
          </div>
      ...
      <div className="p-5 sm:p-6 flex flex-col">
        ...
        <h3
          className="mb-3 transition-colors group-hover:[color:var(--public-primary)] leading-tight"
