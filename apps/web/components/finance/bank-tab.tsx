...
import type { BefitetesRow, KiadasRow, BankAccount } from '@/lib/constants/finance'
import { Landmark, ArrowUpRight, ArrowDownRight, ArrowLeftRight, Building2, AlertTriangle, FileSpreadsheet, Plus, Pencil, Ban, RotateCcw } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { toast } from 'sonner'
// FxRevaluationDialog: az évvégi FX átértékelést a Számadás véglegesítő
// wizard kezeli (auto ellenőrzéssel), ezért a bank-tab nem importálja.
...
function isCashTransactionType(value: string | null): boolean {
...
export function BankTab({
  ...
  useEffect(() => {
    ...
    void Promise.all(
      bankAccounts.map((b) => getBankszamlaNyitoEgyenleg(b.id, currentYear).then((res) => [b.id, res.data ?? null] as const)),
    ).then((pairs) => {
      if (cancelled) return
  ...
  const bankRows = useMemo(() => {
    ...
    incomeRecords.forEach(record => {
      ...
      const hasSomeSource = !!record.id_szemely || !!record.id_csalad
      ...
      const hasMissingPerson = requiresPerson
        ...
        : !hasSomeSource // csak akkor hiányos, ha SEM személy, SEM forrás nincs
      ...
      rows.push({
        ...
        bankszamlaId: (record as unknown as { bankszamla_id?: number | null }).bankszamla_id ?? null,
      })
    })
  ...
  // Bankszámla-szintű szűrés: ha adott bankszámla ki van választva, csak azt
  // mutatjuk; „all" esetén mindet. A tranzakciók szerkezete már tartalmazza
  // a bankszamlaId-t az aktualizált sorrakásznek köszönhetően.
  ...
  const { displayRows, openingBalance, closingBalance, monthIncome, monthExpense } = useMemo(() => {
    ...
    const openingBase = selectedBankFilter === 'all'
      ? carryoverBank
  ...
  }, [filteredBankRows, carryoverBank, monthFilter, sortBy, sortDir, selectedBankFilter, nyitoMap])
    ...
    <div className="space-y-4">
      ...
      {bankAccounts.length === 0 && congregationId && (
        <div className="card-raised p-6 sm:p-8 text-center border-2 border-dashed border-slate-200 bg-gradient-to-br from-white to-emerald-50/30">
          ...
          <h3 className="text-sm font-semibold text-slate-700 mb-1">Még nincs bankszámla</h3>
          <p className="text-xs text-slate-500 mb-4 max-w-md mx-auto">
            Vegyen fel bankszámlát a pénzügyi könyveléshez. Többféle devizát (RON, EUR, HUF, stb.)
            is kezelhet egymás mellett.
          </p>
          <Button
            onClick={() => {
              setEditingAccount(null)
            ...
            className="rounded-xl bg-emerald-600 text-white hover:bg-emerald-700 shadow-sm"
          >
      ...
      {bankAccounts.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {bankAccounts.map(account => {
              return (
                <div key={account.id} className="card-raised flex flex-col gap-2 p-4 group">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-1">
                        <p className="truncate text-sm font-semibold text-slate-700">{account.bank_neve}</p>
                        {congregationId && (
                          <button
                            type="button"
                            onClick={() => {
                      ...
                      {(() => {
                        ...
                        return (
                          <p className="text-[11px] text-amber-700 italic">
                            Az év eleji nyitó egyenleg még nincs rögzítve a {currentYear}. évre — tölts be egy banki Excel-t.
                          </p>
                        )
                      })()}
                    </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <MiniKpi label="Nyito egyenleg" value={formatCurrency(openingBalance)} color="text-slate-700" icon={<Landmark className="h-4 w-4" />} />
        <MiniKpi label="Bevetel" value={`+${formatCurrency(monthIncome)}`} color="text-emerald-600" icon={<ArrowUpRight className="h-4 w-4" />} />
        <MiniKpi label="Kiadas" value={`-${formatCurrency(monthExpense)}`} color="text-red-500" icon={<ArrowDownRight className="h-4 w-4" />} />
        <MiniKpi
          label="Zaro egyenleg"
      ...
      {displayRows.length === 0 ? (
        <div className="card-raised p-8 text-center">
          ...
          <p className="text-sm text-slate-400">
            Nincs banki forgalom {monthFilter !== 'all' ? 'ebben a hónapban' : 'ebben az évben'}.
          </p>
        </div>
      ) : (
            ...
            <div key={monthIdx} className="space-y-2">
              ...
              <div className="flex items-center justify-between rounded-2xl bg-gradient-to-r from-slate-100 to-slate-50 px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-500">{group.rows.length} tétel</span>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="border-b border-slate-100 bg-slate-50/80">
                      <tr>
                        <BankSortableTh col="datum" sortBy={sortBy} sortDir={sortDir} onClick={() => toggleSort('datum')}>Dátum</BankSortableTh>
                        <BankSortableTh col="jogcim" sortBy={sortBy} sortDir={sortDir} onClick={() => toggleSort('jogcim')}>Jogcím</BankSortableTh>
                        <BankSortableTh col="iratszam" sortBy={sortBy} sortDir={sortDir} onClick={() => toggleSort('iratszam')} className="hidden md:table-cell">Iratszám</BankSortableTh>
                        <BankSortableTh col="partner" sortBy={sortBy} sortDir={sortDir} onClick={() => toggleSort('partner')}>Partner</BankSortableTh>
                        <BankSortableTh col="osszeg" sortBy={sortBy} sortDir={sortDir} onClick={() => toggleSort('osszeg')} align="right">Bevétel</BankSortableTh>
                        <BankSortableTh col="osszeg" sortBy={sortBy} sortDir={sortDir} onClick={() => toggleSort('osszeg')} align="right">Kiadás</BankSortableTh>
                        <th className="p-2.5 text-right text-xs font-medium text-slate-500 w-20">Művelet</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {group.rows.map(row => {
                        const missingItems: string[] = []
                        if (row.hasMissingPerson) missingItems.push('nincs személy/család hozzárendelve')
                        if (row.hasMissingCategory) missingItems.push('nincs költségvetési cél')
                        ...
                        const rowBorder = row.stornozott
                          ...
                          : row.isBm
                            ...
                            : hasMissing
                              ...
                              : undefined
                        const textStorno = row.stornozott ? 'line-through decoration-red-400/80 text-slate-400' : ''
                        return (
                        <tr
                          key={`${row.type}-${row.id}`}
                          ...
                          style={rowBorder}
                        >
                          <td className={`whitespace-nowrap p-2.5 text-xs text-slate-500 ${textStorno}`}>{row.datum?.split('T')[0]}</td>
                          <td className="p-2.5">
                            <div className="flex items-center gap-1.5">
                              ...
                              {row.isBm && <ArrowLeftRight className="h-3.5 w-3.5 shrink-0 text-violet-500" />}
                              <span className={`font-medium ${textStorno} ${!row.stornozott && (row.isBm ? 'text-violet-700' : row.hasMissingCategory ? 'text-amber-700' : 'text-slate-700')}`}>
                                {row.celNev || (row.hasMissingCategory ? '⚠ Nincs cél' : '—')}
                              </span>
                            </div>
                          ...
                          <td className={`hidden p-2.5 text-xs text-slate-400 md:table-cell ${textStorno}`}>{row.iratszam || '—'}</td>
                          <td className="p-2.5 text-xs font-medium">
                            <span className={`${textStorno || (row.hasMissingPerson ? 'text-amber-700' : 'text-slate-700')}`}>
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onClick}
        className={`inline-flex items-center gap-1 hover:text-slate-800 transition-colors ${active ? 'text-violet-700 font-semibold' : ''}`}
      >
        {children}
