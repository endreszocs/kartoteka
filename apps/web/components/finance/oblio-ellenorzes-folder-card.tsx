...
import { Calendar, FolderOpen, RefreshCw, AlertTriangle, FolderCog, ShieldAlert, CheckCircle2, Copy, Check, RotateCw } from 'lucide-react'
import { useState } from 'react'
import { toast } from 'sonner'
import { Button } from '@/components/ui/button'
import type { OblioFolderStatus } from '@/lib/finance/oblio/oblio-folder'
      ...
      <div className="card-raised border border-cyan-200 bg-cyan-50/40 p-5 sm:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <p className="font-semibold text-slate-800">Helyi KARTOTEKA mappa beállítása szükséges</p>
              <p className="text-sm leading-6 text-slate-600">
          ...
          <Button onClick={onOpenSettings} className="rounded-xl bg-cyan-600 hover:bg-cyan-700">
            Mappa beállítása
          </Button>
        </div>
      </div>
    ...
    return (
      <div className="card-raised border border-amber-200 bg-amber-50/40 p-5 sm:p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <Button onClick={onRefresh} disabled={isRefreshing} className="rounded-xl bg-amber-600 hover:bg-amber-700">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 space-y-2 w-full">
            ...
            {/* Pontos, másolható útvonal */}
            <PathDisplay path={fullPath} />
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center shrink-0">
          ...
          <div className="flex flex-col gap-2">
            <Button
              ...
              className="rounded-xl bg-cyan-600 hover:bg-cyan-700 shadow-sm"
            >
              <RefreshCw className={`mr-1.5 size-4 ${isRefreshing ? 'animate-spin' : ''}`} />
            ...
            {onReprocessArchive && (
              <Button
                ...
                variant="outline"
                size="sm"
                className="rounded-xl border-amber-300 text-amber-800 hover:bg-amber-50"
...
function PathDisplay({ path }: { path: string }) {
  ...
  async function handleCopy() {
    try {
      ...
      setCopied(true)
      toast.success('Útvonal a vágólapra másolva.')
      setTimeout(() => setCopied(false), 2000)
    } catch {
      toast.error('A másolás nem sikerült — másold ki kézzel.')
    }
  }

  return (
    <div className="flex items-center gap-2 rounded-xl bg-white border border-cyan-100 px-3 py-2">
      <code className="font-mono text-xs text-cyan-900 break-all flex-1 min-w-0">
        {path}
      </code>
      <button
        type="button"
