...
export function DebtTabV2({
  ...
  const modeLabel = debtCalcMode === 'aktualis' ? 'Aktuális év szerinti besorolás' : 'Akkori év szerinti besorolás'
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
        <KpiCard color="amber" icon={<AlertTriangle className="h-5 w-5 text-white" />} value={String(debtors.length)} label="Hátralékos" />
        <KpiCard color="emerald" icon={<CheckCircle className="h-5 w-5 text-white" />} value={String(settled.length)} label="Rendezett" />
        <KpiCard color="sky" icon={<ShieldCheck className="h-5 w-5 text-white" />} value={String(exempted.length)} label="Felmentett" />
        <KpiCard color="blue" icon={<User className="h-5 w-5 text-white" />} value={`${formatCurrency(yearlyFee)} RON`} label={`Éves járulék (${currentYear})`} />
        <KpiCard
          color="orange"
          icon={<Building2 className="h-5 w-5 text-white" />}
          value={`${formatCurrency(totalRentalDebt)} RON`}
          label={`Bérleti hátralék${rentalWithDebt.length ? ` (${rentalWithDebt.length})` : ''}`}
        />
      </div>
      ...
      <div className="card-raised flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500">
            A lista csak az egyházfenntartási befizetésekkel számol, és figyelembe veszi az éves kedvezményeket, a határidős kedvezményt és a felmentéseket is.
          </p>
        </div>
      ...
      {yearlyFee === 0 && (
        <div className="card-raised p-6 text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">Kérjük, állítsa be a Beállítások menüben a járulék összegét.</p>
        </div>
      )}
        ...
        <div className="card-raised overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/80 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-slate-700">Egyházfenntartási státusz tagonként</span>
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="border-b border-slate-100 bg-white/85">
                <tr>
                  ...
                  <th className="p-3 text-left text-xs font-medium text-slate-500">Kedvezmény / szabály</th>
                </tr>
              </thead>
                  ...
                  <tr key={row.memberId} className="hover:bg-slate-50/70">
                    ...
                    <td className="p-3">
                      <span className={statusClassName(row.status)}>
                        {row.status === 'hatralekos' ? 'Hátralékos' : row.status === 'rendezve' ? 'Rendezett' : 'Felmentett'}
                      </span>
                    </td>
                    <td className="p-3 text-right text-slate-600">{formatCurrency(row.expected)} RON</td>
                    <td className="p-3 text-right text-slate-600">{formatCurrency(row.paid)} RON</td>
                    <td className={`p-3 text-right font-semibold ${row.debt > 0 ? 'text-red-500' : 'text-emerald-600'}`}>
                      ...
                      {row.appliedRules.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5">
                          {row.appliedRules.map((rule) => (
                            <span key={rule} className="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-medium text-sky-700">
                              {rule}
                            </span>
                          ))}
                        </div>
      ...
      {yearlyFee > 0 && debtRows.length === 0 && (
        <div className="card-raised p-8 text-center">
          ...
          <p className="text-sm font-medium text-emerald-600">Még nincs aktív, számolható járuléksor ehhez az évhez.</p>
          <p className="mt-1 text-xs text-slate-400">Ha vannak látható tagok és beállított éves járulék, itt fog megjelenni a részletes lista.</p>
        </div>
      )}
        ...
        <div className="card-raised overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 bg-orange-50/60 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-slate-700">Bérleti szerződések — hátralék</span>
            </div>
          ...
          <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-2 text-xs text-slate-500">
            A hátralékot a rendszer az utolsó 2 évre számolja (szerződés szerinti arányosítással és a befizetések dupla-párosításával — személy ID VAGY bérlő neve alapján).
          </div>
        </div>
      )}
    </div>
...
function statusClassName(status: DebtRow['status']) {
  if (status === 'hatralekos') return 'inline-flex rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-600'
  if (status === 'rendezve') return 'inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-600'
