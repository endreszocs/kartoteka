...
/**
 * Pénzügy Súgó — lelkészbarát, kategorizált magyarázat a Pénzügy modul
 * minden funkciójáról.
 *
 * Felépítés:
...
import { FinanceSugoChecklist } from '@/components/finance/finance-sugo-checklist'
...
type ColorKey = 'blue' | 'emerald' | 'violet' | 'pink' | 'amber' | 'cyan' | 'orange' | 'slate' | 'rose' | 'teal'
...
const PALETTE: Record<ColorKey, { ring: string; bg: string; text: string; gradient: string; border: string; bgLight: string }> = {
  blue: { ring: 'ring-blue-200', bg: 'bg-blue-50', bgLight: 'bg-blue-50/30', text: 'text-blue-700', gradient: 'from-blue-500 to-indigo-600', border: 'border-blue-200' },
  emerald: { ring: 'ring-emerald-200', bg: 'bg-emerald-50', bgLight: 'bg-emerald-50/30', text: 'text-emerald-700', gradient: 'from-emerald-500 to-green-600', border: 'border-emerald-200' },
  violet: { ring: 'ring-violet-200', bg: 'bg-violet-50', bgLight: 'bg-violet-50/30', text: 'text-violet-700', gradient: 'from-violet-500 to-purple-600', border: 'border-violet-200' },
  pink: { ring: 'ring-pink-200', bg: 'bg-pink-50', bgLight: 'bg-pink-50/30', text: 'text-pink-700', gradient: 'from-pink-500 to-rose-600', border: 'border-pink-200' },
  amber: { ring: 'ring-amber-200', bg: 'bg-amber-50', bgLight: 'bg-amber-50/30', text: 'text-amber-700', gradient: 'from-amber-500 to-orange-600', border: 'border-amber-200' },
  cyan: { ring: 'ring-cyan-200', bg: 'bg-cyan-50', bgLight: 'bg-cyan-50/30', text: 'text-cyan-700', gradient: 'from-cyan-500 to-teal-600', border: 'border-cyan-200' },
  orange: { ring: 'ring-orange-200', bg: 'bg-orange-50', bgLight: 'bg-orange-50/30', text: 'text-orange-700', gradient: 'from-orange-500 to-red-600', border: 'border-orange-200' },
  slate: { ring: 'ring-slate-200', bg: 'bg-slate-50', bgLight: 'bg-slate-50/30', text: 'text-slate-700', gradient: 'from-slate-500 to-slate-700', border: 'border-slate-200' },
  rose: { ring: 'ring-rose-200', bg: 'bg-rose-50', bgLight: 'bg-rose-50/30', text: 'text-rose-700', gradient: 'from-rose-500 to-pink-600', border: 'border-rose-200' },
  teal: { ring: 'ring-teal-200', bg: 'bg-teal-50', bgLight: 'bg-teal-50/30', text: 'text-teal-700', gradient: 'from-teal-500 to-cyan-600', border: 'border-teal-200' },
}
...
export function FinanceSugoTab() {
  ...
  const activeSectionLabel =
    SECTIONS.find((s) => s.key === activeTopic.sectionKey)?.label

  async function handlePrintTopic(mode: 'preview' | 'pdf') {
    setPrinting(mode)
    try {
      const html = buildTopicPdfHtml({
      ...
      if (mode === 'pdf') {
        ...
        await printToPdf(html, filename, { orientation: 'portrait', margin: [15, 15], format: 'a4' })
      ...
      <div className="card-raised p-5 sm:p-6 bg-gradient-to-br from-teal-50/40 to-cyan-50/30 border border-teal-100">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-teal-700/70">Pénzügy modul</p>
            <h2 className="font-heading text-2xl sm:text-3xl text-slate-800">Súgó és magyarázat</h2>
            <p className="mt-1.5 text-sm leading-6 text-slate-600 max-w-3xl">
              Itt megtalálod minden funkció lelkészbarát magyarázatát — kategorizálva, lépésről-lépésre,
              tippekkel és példákkal. Bal oldalon böngéssz a témák között, jobbra olvasd a részleteket.
            </p>
          </div>
        </div>
      </div>

      {/* A korábbi PDF referencia-könyvtár áthelyezve — most minden topic-nál
          saját kis nyomtató ikon van, amely csak az adott részt nyomtatja. */}
            ...
            <div key={section.key} className="card-raised p-3">
              <div className="flex items-center gap-2 mb-2 px-1">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-bold uppercase tracking-wider text-slate-700">{section.label}</p>
        ...
        <div className="space-y-4">
          ...
          <div className={`card-raised border ${palette.border} ${palette.bgLight} p-5 sm:p-6`}>
            <div className="flex items-start gap-3">
              <span className={`inline-flex size-12 items-center justify-center rounded-2xl bg-gradient-to-br ${palette.gradient} text-white shadow-md shrink-0`}>
          ...
          {activeTopic.howItWorks && activeTopic.howItWorks.length > 0 && (
            <CardSection palette={palette} title="Hogyan működik" icon={<Repeat className="size-4" />}>
                ...
                {activeTopic.howItWorks.map((step, i) => (
                  <li key={i} className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {step.hint && (
                        <p className={`mt-1 text-xs leading-5 ${palette.text} flex items-start gap-1`}>
                ...
                >
                  <span
                    className={`inline-flex size-8 shrink-0 items-center justify-center rounded-xl ${
                      tip.kind === 'warning' ? 'bg-amber-100 text-amber-700' : `${palette.bg} ${palette.text}`
                    }`}
                  >
                    {tip.kind === 'warning' ? <AlertCircle className="size-4" /> : <Lightbulb className="size-4" />}
                  </span>
                  <div className="flex-1">
                    <p
          ...
          {activeTopic.examples && activeTopic.examples.length > 0 && (
            <CardSection palette={palette} title="Példák" icon={<FileSpreadsheet className="size-4" />}>
              <div className="space-y-3">
                {activeTopic.examples.map((ex, i) => (
                  <div key={i} className="rounded-xl border border-slate-200 bg-white p-3">
                    <p className="text-xs uppercase tracking-wide font-bold text-slate-500 mb-1">Helyzet</p>
                    <p className="text-sm text-slate-700 italic">&bdquo;{ex.situation}&rdquo;</p>
                    <div className="my-2 h-px bg-slate-100" />
                    <p className={`text-xs uppercase tracking-wide font-bold mb-1 ${palette.text}`}>Megoldás</p>
          ...
          <div className="text-center text-xs text-slate-400 pt-2 pb-4">
            ...
            <span>{allTopics.length} téma · {SECTIONS.length} kategória</span>
          </div>
        </div>
      </div>

      {/* ÉLŐ CHECKLIST — a súgó legvégén, teljes szélességben */}
      <FinanceSugoChecklist />
    </div>
  )
}
  ...
  return (
    <div className="card-raised p-4 sm:p-5">
      <h4 className="font-heading text-base text-slate-800 mb-3 flex items-center gap-2">
        <span className={`inline-flex size-7 items-center justify-center rounded-lg ${palette.bg} ${palette.text}`}>
          {icon}
        </span>
        {title}
      </h4>
