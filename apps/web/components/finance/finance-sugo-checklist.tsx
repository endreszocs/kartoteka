...
/**
 * Pénzügyi súgó végén megjelenő ÉLŐ CHECKLIST.
 *
 ...
 *
 * Cél: vizuálisan lássa, mi van rendben és mi vár még rá az év végi
 * zárás előtt. A megjelenített progress bar motiváló.
 */
...
interface ChecklistItem {
  ...
  relatedTopic?: string // súgó-topic kulcs, amire linkel
}
...
export function FinanceSugoChecklist() {
  ...
  function resetAll() {
    if (!confirm(`Biztosan törlöd az összes pipát a ${year}. évre?`)) return
    setChecked(new Set())
    try {
  ...
  return (
    <div className="card-raised bg-gradient-to-br from-emerald-50/50 to-teal-50/30 border-emerald-200 p-5 sm:p-6">
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between text-xs mb-1.5">
            <span className="text-slate-600">
              <strong className="text-slate-800">{checkedCount}</strong> / {totalCount} feladat kipipálva
            </span>
            <span className={`font-bold ${percent === 100 ? 'text-emerald-700' : 'text-slate-700'}`}>
              {percent}%
            </span>
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-start gap-3 rounded-xl p-2.5 text-left transition-colors ${
                        isChecked ? 'bg-emerald-50/70 hover:bg-emerald-100/70' : 'hover:bg-slate-50'
                      }`}
                    >
                      <span
                      ...
                      >
                        {isChecked && <Check className="size-3.5 text-white" strokeWidth={3} />}
                      </span>
                      <div className="flex-1 min-w-0">
                        ...
                        <p
                          className={`text-xs mt-0.5 leading-relaxed ${
                            isChecked ? 'text-emerald-700/80 line-through decoration-emerald-400/60' : 'text-slate-500'
                          }`}
                        >
                          {item.description}
                        </p>
      ...
      {mounted && evvegiReady && (
        <div className="mt-5 rounded-2xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50 to-teal-50 p-5 text-center">
          ...
          <p className="text-sm text-emerald-800 mb-3">
            Minden év végi pont ki van pipálva. Nyugodtan léphetsz a Számadás fülre,
            és indíthatod a véglegesítést + beküldést az egyházmegyének.
          </p>
          <a
            href="/penzugy?tab=accounting"
      ...
      <div className="mt-4 flex items-start gap-2 text-xs text-slate-500 italic">
        ...
        <p>
          A pipálások csak a böngésződben tárolódnak — nem szinkronizálódnak
          másik eszközzel vagy felhasználóval. Privát, személyes ellenőrző lista.
        </p>
      </div>
    </div>
  )
...
// Placeholder circle ikon — némelyik böngésző nem render-eli az SVG-t, de a Circle import védőháló
