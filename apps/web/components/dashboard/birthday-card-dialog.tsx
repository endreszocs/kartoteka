 ...
 *
 * html2canvas-korlátok (PR-14 tanulságok mintájára): NINCS CSS outline, NINCS
 * column-count, NINCS radial-gradient — helyettük beágyazott keret-div,
 * flex-oszlopok, linear-gradient. A rejtett példány position:absolute +
 * top:-20000px (fixed a transformált DialogContent-en belül fantom-görgetést
 * adna; a negatív irányú túllógás nem növeli a scroll-területet).
 */
...
export interface BirthdayCardPerson {
  ...
  month: number
  day: number
  /** A szűrt találatban ehhez az évfordulóhoz tartozó betöltött életkor */
  age: number
}
...
interface BirthdayCardDialogProps {
  ...
  /** A lista-dialógus AKTUÁLIS szűrt találatai (előfordulás szerint rendezve) */
...
interface CardArtProps {
  mode: CardMode
  person: BirthdayCardPerson | null
  listTitle: string
  listPeople: BirthdayCardPerson[]
  listOverflow: number
  showAge: boolean
  message: string
  congregationName: string
...
/**
 * A kártya rajzolata — közös az előnézethez és a képkészítéshez. TOP-LEVEL
 * komponens (nem a dialóguson belül definiált), különben minden billentyű-
 * leütésre teljesen újra-mountolódna (logó-villogás, felesleges munka).
 */
function CardArt({ mode, person, listTitle, listPeople, listOverflow, showAge, message, congregationName, logoDataUrl }: CardArtProps) {
  ...
  const singleDateLabel = person ? `${MONTH_NAMES[person.month]} ${person.day}.` : ''
  return (
    <div
      style={{
        width: 1080,
        // FIX magasság (nem minHeight): a letöltött kép mérete kiszámítható
        // (1080×1080 / 1080×1350 — közösségi média-barát), és az előnézet
        // aspect-ratio-ja pixelre egyezik vele (nincs levágott alj).
        height: mode === 'single' ? 1080 : 1350,
        overflow: 'hidden',
      ...
      >
        <div
          style={{
            ...
            borderRadius: 18,
            padding: mode === 'single' ? '64px 72px' : '48px 60px',
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'center',
            textAlign: 'center',
        ...
        >
          ...
          <div style={{ width: 220, height: 2, background: 'linear-gradient(90deg, transparent, #f3c061, transparent)', margin: '28px 0' }} />
          ...
          {mode === 'single' && person ? (
            ...
            <>
              ...
              <div style={{ marginTop: 30, width: '100%', display: 'flex', gap: 48, textAlign: 'left' }}>
                {columns.map((col, ci) => (
                  <div key={ci} style={{ flex: 1, fontSize: colCount === 2 ? 27 : 32, lineHeight: 1.72 }}>
                    {col.map((p) => (
          ...
          {message.trim() && (
            <div style={{ fontSize: 30, lineHeight: 1.5, color: '#f3f1ec', maxWidth: 780, marginBottom: 24 }}>
              {message.trim()}
            </div>
          )}
...
export function BirthdayCardDialog({
  ...
  // gyors kattintás a fallback ⛪ emojival készítené a képet.
  useEffect(() => {
    if (!open) return
    setMode(initialPersonId ? 'single' : 'list')
    setPersonId(initialPersonId != null ? String(initialPersonId) : (people[0] ? String(people[0].id) : null))
    setShowAge(true)
    setMessage('')
    let cancelled = false
    if (congregationLogo) {
  ...
  // 2026-08-02 (review-fix): a dashboard útvonalon a tag-id futásidőben SZÁM
  // (a típus szerint string) — a String()-normalizálás nélkül a választó
  // sosem talált volna személyt.
  const person = useMemo(
    () => people.find((p) => String(p.id) === String(personId)) ?? people[0] ?? null,
    [people, personId],
  )

  // A listás kártya címe: ha minden találat egy hónapba esik, a hónap neve;
  // különben általános cím. (Évszámot szándékosan nem írunk — évhatár-átnyúló
  // szűrésnél félrevezető lenne.)
    ...
    try {
      ...
      if (typeof navigator.canShare === 'function' && navigator.canShare(shareData)) {
        try {
          ...
          if (e instanceof Error && e.name === 'AbortError') return // user mégse
          // Néhány böngésző a hosszú render után már nem tekinti felhasználói
          // gesztusnak a hívást (NotAllowedError) → letöltés-fallback.
          downloadBlob(blob)
          toast.info('A közvetlen megosztás nem indult el — a képet letöltöttük, innen oszthatod meg.')
        }
      } else {
        downloadBlob(blob)
  ...
  const actionsDisabled = !!busy || people.length === 0 || logoLoading

  const cardProps: CardArtProps = {
    mode,
    person,
    listTitle,
    listPeople,
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!busy) onOpenChange(next) }}>
      <DialogContent className="max-h-[calc(100dvh-1rem)] w-[calc(100vw-1rem)] overflow-y-auto overscroll-contain rounded-[1.5rem] sm:max-w-4xl">
        ...
        <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2">
              <Button className="min-h-11 rounded-xl" onClick={() => void handleDownload()} disabled={actionsDisabled}>
                <Download className="size-4" />
                {busy === 'png' ? 'Készítés…' : logoLoading ? 'Címer betöltése…' : 'Kép letöltése (PNG)'}
              </Button>
              <Button variant="outline" className="min-h-11 rounded-xl" onClick={() => void handleShare()} disabled={actionsDisabled}>
                <Share2 className="size-4" />
                {busy === 'share' ? 'Megosztás…' : 'Megosztás'}
              </Button>
              <Button variant="outline" className="min-h-11 rounded-xl" onClick={() => void handlePrint()} disabled={actionsDisabled}>
        ...
        {/* Rejtett, TELJES méretű példány — ebből készül a PNG. absolute +
            negatív top: nem növeli a dialógus görgethető területét. */}
        <div style={{ position: 'absolute', left: 0, top: -20000, pointerEvents: 'none' }} aria-hidden="true">
          <div ref={captureRef}>
            <CardArt {...cardProps} />
          </div>
        </div>
