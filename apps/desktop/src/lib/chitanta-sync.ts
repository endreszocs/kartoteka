...
export async function pushPendingChitantas(
  ...
  for (const mutation of chitantaMutations) {
    ...
    // Max-retry check: ha eddig 5-ször próbáltuk és most is nekifutnánk,
    // billentsük conflict-re (a 6. próbálkozás felesleges).
    if (mutation.attempts >= MAX_ATTEMPTS) {
      try {
        await backend.markChitantaConflict(
          mutation.pk,
          `Max próbálkozás elérve (${mutation.attempts}). Utolsó hiba: ${mutation.lastError ?? 'ismeretlen'}. Kérlek nyisd meg a chitantát és ellenőrizd a sorszámot.`,
        )
