...
**Megvalósítás-haladás:** F1 (audit-alap) ✅ · F2a (lelkészi szolgálati napló) ✅ · F2b (fiók végleges törlése = GDPR-anonimizálás) ✅ · **F3a (átadás INDÍTÁSA + értesítés)** ✅ — mind deployolva. Az `initiate_congregation_transfer()` RPC + `congregation_transfers`/`congregation_remarks` táblák; a távozó lelkész a „Lelkészek" fülön indítja → admin + egyházmegyei számvevő értesül (in-app + email). **F3b-1 (jóváhagyás/meghagyás workflow)** ✅ deployolva: `transfer_approve()` (admin/számvevő auto-detekt + „nincs számvevő → admin elég"), `transfer_add_remark()`, `get_open_transfer_for_congregation()` RPC-k; a „Lelkészek" fülön a nyitott átadás állapota + jóváhagy-gomb/meghagyás (admin/számvevőnek). SQL: `2026-06-05i-transfer-review.sql`. **F3c (végrehajtás)** ✅ deployolva: `complete_congregation_transfer()` RPC — a felelős átállítása (régi tenure zár 'transfer', új nyit; régi pastor congregation_id ürül + szerep revoke; új pastor congregation_id + lelkész profile_role + aktív), az átadás 'completed'. A „ready" állapotban az admin megadja a bejövő lelkész emailjét: ha létezik → hozzárendelés + email; ha nem → meghívó email (regisztráljon). A teljes átadás-kör KÉSZ (indít → értesít → review/jóváhagy/meghagyás → ready → admin véglegesít → új lelkész). **F4a (1 éve inaktív gyülekezetek)** ✅ deployolva: `congregations.status` + `last_activity_at` oszlopok + `mark_inactive_congregations()` (lelkész last_seen + befizetés + created alapján) + napi pg_cron; „Inaktív" badge a gyülekezeti dialógusban. SQL: `2026-06-05k-inactive-congregations.sql`. **F4b-1 (sor-szintű audit — DB-réteg)** ✅ deployolva: `audit` séma + `audit.record_version` (old/new JSONB + aktor + congregation_id) + `audit.log_change()` trigger a kulcstáblákon (befizetes, kiadas, szemely, profiles, profile_roles, congregations, bealitas, jarulek_kedvezmeny, felmentes) + RLS (admin/számvevő/lelkész scope) + `get_record_audit()` lekérdező RPC. Az aktor: `app.actor_id` GUC VAGY JWT sub. SQL: `2026-06-05n-row-audit.sql`. A napló MOST elkezd gyűlni. Hátralévő: **F4b-2** (auditor-UI a napló megtekintéséhez), **F3b-2** (read-only review-belépés — kényelmi), opcionálisan az `app.actor_id` injektálása a service-role írás-RPC-kbe (pontosabb aktor).
