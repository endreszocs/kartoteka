-- ═══════════════════════════════════════════════════════════════════════════
--  TAGNYILVÁNTARTÁS + ANYAKÖNYV — ÉLŐ ÁLLAPOTFELMÉRÉS (CSAK OLVAS)
--  2026-09-04 — Futtatja: Endre (Supabase Studio SQL Editor)
--
--  ⚠️ EZ A FÁJL SZÁNDÉKOSAN EGYETLEN LEKÉRDEZÉST TARTALMAZ.
--  Az eredeti (2026-09-02-i) fájlban három blokk állt egymás alatt, és a
--  Supabase szerkesztő MINDIG csak az UTOLSÓ eredmény-rácsot mutatja — így
--  kétszer is a 2. blokk (H-audit / G-tenant) eredménye jött vissza, az
--  állapotfelmérésé pedig felülíródott. Itt nincs mit kijelölni:
--
--      JELÖLÉS NÉLKÜL nyomj Run-t. Egy rács, ~26 sor.
--
--  SEMMIT NEM MÓDOSÍT: egyetlen SELECT, se INSERT, se UPDATE, se DDL.
--
...
--
--  MIRE VÁROK BELŐLE ELSŐSORBAN:
--   · B-trigger — van-e egyáltalán audit-trigger az anyakönyvi táblákon.
--     (A 2. blokk szerint azoknak NULLA audit-soruk van, miközben a
--      `szemely`-en 4965 áll. Ez „nincs trigger" vagy „van, csak nem
--      változott semmi"? Ez a sor dönti el.)
--   · C-függvény — mely élő függvények futnak, és milyen kapukkal.
--   · D-policy — a tényleges RLS-szabályok szövege.
-- ═══════════════════════════════════════════════════════════════════════════
