...
/**
 * Celebrations — „Ma köszöntjük" kártya (KÖZÖS, web-azonos).
 *
 * 2026-06-12 (Endre #5 — dashboard-paritás): a webes
 * `apps/web/components/dashboard/celebrations.tsx` VÁLTOZATLAN kinézettel és
 * viselkedéssel ide költözött — a web re-export shimen át, a desktop közvetlen
 * importtal használja. A korábbi (egyszerűsített, Sprint B-s) desktop-változat
 * ezzel kivezetve.
 *
 * Megjelenés és viselkedés (pontos web-tükör):
 *   - Mai születésnaposok (max 5) — Cake-ikon + „X éves" badge
 *   - Mai névnapos tagok (max 5) — Flower2-ikon
 *   - Ha nincs köszöntött, de van mai névnap: „Ma: … — nincs érintett tag."
 *   - Következő 14 nap születésnapjai (max 5 + összlétszám-badge)
 *   - „Lista" gomb → BirthdayListDialog (szűrés + nyomtatás)
 */

import { useState } from 'react'
import { Cake, Flower2, CalendarClock, List } from 'lucide-react'

import { BirthdayListDialog, type CelebrationsMember } from './BirthdayListDialog'

// A webes lib/constants/dashboard HU_MONTHS_SHORT tükre (web-azonos feliratok)
...
export interface CelebrationsProps {
  ...
  /** Desktop: iframe-es nyomtató átadása a lista-dialógusnak (web: hagyd üresen). */
