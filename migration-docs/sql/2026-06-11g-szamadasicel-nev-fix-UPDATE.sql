-- 2026-06-11g — szamadasicel NÉV-FIX UPDATE
-- ⚠️ ELŐBB futtasd a 2026-06-11f-…-DIFF.sql riportot és nézd át az eltéréseket!
-- A hivatalos EREK Excel 2026 katalógusa alapján a Kartotéka szamadasicel
-- magyar (nev) + román (nevro) neveit a HIVATALOS Excel-nevekre igazítja, KÓD (id)
-- szerint. Csak a neveket írja — összeget, struktúrát, aktív-flaget NEM érint.
-- Idempotens: csak ott ír, ahol tényleg eltér. RETURNING-gel mutatja, mit módosított.
...
UPDATE public.szamadasicel s
SET nev   = e.nev_hu,
    nevro = e.nev_ro
FROM excel e
WHERE s.id = e.kod
  AND (s.nev IS DISTINCT FROM e.nev_hu OR s.nevro IS DISTINCT FROM e.nev_ro)
RETURNING s.id AS kod, s.nev AS uj_nev, s.nevro AS uj_nevro;
