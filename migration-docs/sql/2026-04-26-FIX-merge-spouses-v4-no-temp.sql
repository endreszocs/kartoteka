-- KARTOTEKA — Hiányzó házastársak v4 (NO TEMP — Studio batch-stable)
-- Dátum: 2026-04-26
-- Futtatja: Endre (Supabase Studio SQL Editor)
--
-- A v3 hibája: Studio batch-ekben futtat, a TEMP tábla nem éli túl.
-- A v4 perszisztens táblát használ: `_merge_run_log` — ez minden futás után
-- DROP-olódik a script végén.

-- ════════════════════════════════════════════════════════════════════════════
-- 0. ELŐKÉSZÍTÉS — log-tábla létrehozása (NEM TEMP, perzisztens)
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
-- 1. DIAGNOSZTIKA — candidate_pairs (mit lát a NAMEMATCH algoritmus)
-- ════════════════════════════════════════════════════════════════════════════

WITH single_parent_at_addr AS (
    SELECT
        c.id AS csalad_id, c.c_utcaid, c.c_szam,
        head.id AS head_szemely_id, head.csaladnev, head.k_nev, head.szcs_nev,
...
candidate_pairs AS (
    SELECT
        ...
        f.nev AS ferj_nev, n.nev AS no_nev,
        f.c_utcaid, f.c_szam,
...
SELECT
    a.name AS utca, cp.c_szam,
    cp.ferj_nev, cp.no_nev,
    cp.ferj_norm, cp.no_norm, cp.no_szcs_nev,
    cp.match_reason,
...
-- ════════════════════════════════════════════════════════════════════════════
-- 2. MERGE — STRICT (1-1 single-parent ugyanazon címen)
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    LOOP
        BEGIN
            UPDATE public.csalad SET id_no = pair_rec.no_szemely_id WHERE id = pair_rec.ferj_csalad_id;
            UPDATE public.gyerek SET id_csalad = pair_rec.ferj_csalad_id WHERE id_csalad = pair_rec.no_csalad_id;
            DELETE FROM public.csalad WHERE id = pair_rec.no_csalad_id;
            UPDATE public.szemely SET csaladfo = false WHERE id = pair_rec.no_szemely_id;
            INSERT INTO public._merge_run_log (phase, ferj_csalad_id, no_csalad_id, action)
                VALUES ('STRICT', pair_rec.ferj_csalad_id, pair_rec.no_csalad_id, 'merged');
        EXCEPTION WHEN unique_violation THEN
            INSERT INTO public._merge_run_log (phase, ferj_csalad_id, no_csalad_id, action, notes)
                VALUES ('STRICT', pair_rec.ferj_csalad_id, pair_rec.no_csalad_id, 'skipped',
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            UPDATE public.szemely SET csaladfo = false WHERE id = pair_rec.no_szemely_id;
            INSERT INTO public._merge_run_log (phase, ferj_csalad_id, no_csalad_id, action)
                VALUES ('NAMEMATCH', pair_rec.ferj_csalad_id, pair_rec.no_csalad_id, 'merged');
        EXCEPTION WHEN unique_violation THEN
            INSERT INTO public._merge_run_log (phase, ferj_csalad_id, no_csalad_id, action, notes)
                VALUES ('NAMEMATCH', pair_rec.ferj_csalad_id, pair_rec.no_csalad_id, 'skipped',
                    'UNIQUE: id_no/id_ferfi már használva');
        END;
    END LOOP;
END;
    ...
    LOOP
        ...
        IF cand_count = 1 THEN
            BEGIN
                ...
                END IF;
                INSERT INTO public._merge_run_log (phase, ferj_csalad_id, action, notes)
                    VALUES ('LOOSE', sp_rec.csalad_id, 'linked', 'spouse_id=' || cand_id);
            EXCEPTION WHEN unique_violation THEN
                INSERT INTO public._merge_run_log (phase, ferj_csalad_id, action, notes)
                    VALUES ('LOOSE', sp_rec.csalad_id, 'skipped', 'UNIQUE: spouse_id=' || cand_id);
            END;
        ELSIF cand_count > 1 THEN
            INSERT INTO public._merge_run_log (phase, ferj_csalad_id, action, notes)
                VALUES ('LOOSE', sp_rec.csalad_id, 'skipped_ambiguous', cand_count || ' jelölt');
        END IF;
    END LOOP;
END;
$loose_link$;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 5. EREDMÉNY-ÖSSZEGZÉS a perzisztens táblából
-- ════════════════════════════════════════════════════════════════════════════

SELECT phase, action, COUNT(*) AS db
FROM public._merge_run_log
GROUP BY phase, action
ORDER BY phase, action;

-- Részletes lista (max 50)
SELECT id, phase, ferj_csalad_id, no_csalad_id, action, notes
FROM public._merge_run_log
ORDER BY id
LIMIT 50;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 7. CLEANUP — log-tábla törlése (futás után, ha már nem kell)
-- ════════════════════════════════════════════════════════════════════════════
-- Ha akarod megőrizni, kommenteld ki:
-- DROP TABLE IF EXISTS public._merge_run_log;
