-- ===================================================================
-- Pénzügyi import RPC v6 — ADVISORY LOCK a párhuzamos futás ellen (2026-08-28)
-- ===================================================================
-- P0-21 (pénzügyi audit, 2026-08-28): a v5 darabszám-alapú dedupja a már
-- COMMITOLT sorokat számolja (READ COMMITTED alatt) — KÉT PÁRHUZAMOS
-- import-hívás nem látja egymás nyitott insertjeit, ezért MINDKETTŐ átengedi
-- ugyanazokat a tételeket → teljes duplikáció, amit az utólagos újraimport
-- SEM gyógyít (a dedup csak kihagy, sosem töröl).
--
-- v6 = a v5 törzse BETŰRE változatlanul + EGYETLEN kiegészítés: a tranzakció
-- elején gyülekezetenkénti advisory lock, amely a második párhuzamos hívást
-- megvárakoztatja, amíg az első commitol — így az már látja a beszúrt
-- sorokat, és a dedup helyesen kihagyja őket.
--
-- ⚠️ A p_user_id-alapú jogosultság-ellenőrzés SZÁNDÉKOSAN VÁLTOZATLAN —
--    annak megerősítése a külön jogosultsági körben történik.
--
-- CREATE OR REPLACE — felülírja a v5-öt. ENDRE FUTTATJA.
-- ===================================================================
--
-- ── Az eredeti v5 fejléc (kontextusnak, változatlanul): ──
-- -- ===================================================================
...
BEGIN
  -- P0-21 (2026-08-28): gyülekezetenkénti sorosítás — a párhuzamos import-
  -- hívások közül a második itt megvárja az első COMMIT-ját, így a
  -- darabszám-alapú dedup már látja a beszúrt sorokat.
